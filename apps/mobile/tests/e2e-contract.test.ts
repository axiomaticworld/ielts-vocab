...
describe('mobile Maestro E2E contract', () => {
  ...
  it('opens the center practice tab as the fullscreen quick practice view', () => {
    ...
    assert.match(practiceActionMenuSource, /ribbonNumber: \{[\s\S]*?minWidth: 80,[\s\S]*?textAlign: 'center',/)
    assert.match(practiceActionMenuSource, /ribbonNumberSlot: \{[\s\S]*?position: 'absolute',/)
    assert.match(practiceActionMenuSource, /ribbonTextPrefix: \{[\s\S]*?right: 156,/)
    assert.match(practiceActionMenuSource, /ribbonTextSuffix: \{[\s\S]*?left: 156,/)
    assert.match(practiceActionMenuSource, /ribbonTextWrap: \{[\s\S]*?position: 'relative',[\s\S]*?width: 236,/)
