...
describe('mobile Maestro E2E contract', () => {
  ...
  it('opens the center practice tab as the fullscreen quick practice view', () => {
    ...
    assert.match(practiceActionMenuSource, /<Text adjustsFontSizeToFit minimumFontScale=\{0\.72\} numberOfLines=\{1\} style=\{styles\.ribbonText\}>/)
    assert.match(practiceActionMenuSource, /今日掌握<Text style=\{styles\.ribbonNumber\}>\{displayedTodayMasteredWords\}<\/Text>词/)
    assert.match(practiceActionMenuSource, /ribbonNumber: \{[\s\S]*?fontSize: 30,/)
    assert.match(practiceActionMenuSource, /ribbonText: \{[\s\S]*?width: '100%',/)
    assert.match(practiceActionMenuSource, /ribbonTextWrap: \{[\s\S]*?width: 230,/)
