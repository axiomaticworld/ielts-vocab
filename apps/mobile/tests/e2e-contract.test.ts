...
describe('mobile Maestro E2E contract', () => {
  ...
  it('opens the center practice tab as the fullscreen quick practice view', () => {
    ...
    assert.match(practiceActionMenuSource, /ribbonNumberSlot: \{[\s\S]*?justifyContent: 'center',[\s\S]*?width: 104,/)
    assert.match(practiceActionMenuSource, /ribbonText: \{[\s\S]*?includeFontPadding: false,[\s\S]*?textAlignVertical: 'center',/)
    assert.match(practiceActionMenuSource, /ribbonTextPrefix: \{[\s\S]*?textAlign: 'right',[\s\S]*?width: 96,/)
    assert.match(practiceActionMenuSource, /ribbonTextSuffix: \{[\s\S]*?textAlign: 'left',[\s\S]*?width: 96,/)
